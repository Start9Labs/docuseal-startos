...
DOCUSEAL_SRC := $(shell find ./docuseal -name \*)
...
docker-images/aarch64.tar: Dockerfile docker_entrypoint.sh $(DOCUSEAL_SRC)
ifeq ($(ARCH),x86_64)
else
...
docker-images/x86_64.tar: Dockerfile docker_entrypoint.sh $(DOCUSEAL_SRC)
ifeq ($(ARCH),aarch64)
else
	mkdir -p docker-images
