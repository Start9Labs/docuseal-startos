...
This repo packages [DocuSeal](https://www.docuseal.com/) for StartOS.
...
DocuSeal runs the official `docuseal/docuseal` image, pinned by tag in the manifest. To track a new upstream release:

1. Bump `dockerVersion` in `startos/manifest/index.ts` to the new upstream tag. The image tag is assembled from `dockerImage` (`docuseal/docuseal`) and `dockerVersion` and supplied to the `main` image via `source.dockerTag`.
